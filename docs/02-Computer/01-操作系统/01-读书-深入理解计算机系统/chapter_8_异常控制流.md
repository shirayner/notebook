...
fork 函数是有趣的（也常常令人迷惑），因为它**只被调用一次，却会返回两次**：一次是在调用进程（父进程）中，一次是在新创建的子进程中。在父进程中，fork 返回子进程的 PID。在子进程中，fork 返回 0。因为子进程的 PID 总是为非零，返回值就提供一个明确的方法来分辨程序是在父进程还是在子进程中执行。
...
![img](./images/01_异常控制流/图8-15中示例程序的进程图.png)
...
![img](./images/01_异常控制流/嵌套fork的进程图.png)
...
![img](./images/01_异常控制流/一个新程序开始时，用户栈的典型组织结构.png)

> 图 8-22 一个新程序开始时，用户栈的典型组织结构

main 函数有 3 个参数：

1. 1.argc，它给出 argv[ ] 数组中非空指针的数量；
2. 2.argv，指向 argv[ ] 数组中的第一个条目；
3. 3.envp，指向 envp[ ] 数组中的第一个条目。
...
![img](./images/01_异常控制流/前台和后台进程组.png)
...
![img](./images/01_异常控制流/信号处理程序可以被其他信号处理程序中断.png)
...
1. 1.处理程序与主程序并发运行，共享同样的全局变量，因此可能与主程序和其他处理程序互相干扰；
2. 2.如何以及何时接收信号的规则常常有违人的直觉；
3. 3.不同的系统有不同的信号处理语义。
...
1. 1.父进程执行 fork 函数，内核调度新创建的子进程运行，而不是父进程。
2. 2.在父进程能够再次运行之前，子进程就终止，并且变成一个僵死进程，使得内核传递一个 SIGCHLD 信号给父进程。
3. 3.后来，当父进程再次变成可运行但又在它执行之前，内核注意到有未处理的 SIGCHLD 信号，并通过在父进程中运行处理程序接收这个信号。
4. 4.信号处理程序回收终止的子进程，并调用 deletejob，这个函数什么也不做，因为父进程还没有把该子进程添加到列表中。
5. 5.在处理程序运行完毕后，内核运行父进程，父进程从 fork 返回，通过调用 add-job 错误地把（不存在的）子进程添加到作业列表中。
...
1. 1.逻辑控制流，它提供给每个程序一个假象，好像它是在独占地使用处理器；
2. 2.私有地址空间，它提供给每个程序一个假象，好像它是在独占地使用主存。
...
Kerrisk 是 Linux 环境编程的完全参考手册【62】。Intel ISA 规范包含对 Intel 处理器上的异常和中断的详细讨论【50】。操作系统教科书中 【102，106，113】包括关于异常、进程和信号的其他信息。W. Richard Stevens 的【111】是一本有价值的和可读性很高的经典著作，是关于如何在应用程序中处理进程和信号的。Bovet 和 Cesati【11】给出了一个关于 Linux 内核的非常清晰的描述，包括进程和信号实现的细节。
